...
main()
{
	...
	ofstream outfile;  
     outfile.open("hypersphere_table_latex.dat");
   
//     outfile << "# Dim 	Nmax 	Vsphere	FracErr" << endl;
     outfile << "# Ntrials = 500000" <<endl<< "Dim & Vcube & Vsphere & Vactual & FracErr" << endl;
...
for (int NDIM=2;NDIM<11;NDIM++){

//int trials[11] = {10, 30, 100, 300, 1000, 3000, 10000, 30000, 100000, 300000, 1000000};
//for (int nt=0; nt<11; nt++){

	double hit,xi,R,Rsq,D,R_D,Vtot,Vsphere;
	double Vtrue=0.0,result,integproduct=1.0;
	int n,i, Ntrials=500000;
...
//	Ntrials = trials[nt];
	...
	Vtot = D;	// Vtot=D for 1-dimensional "hypercube" (a line)
	 for(i=1;i<NDIM;i++){
	        Vtot *= Vtot;  //iterative multiplication to get N-dim hypercube
	    }

	 /* get ratio of volumes */
	 Vsphere = hit/Ntrials*pow(2, NDIM);  // determine the hit ratio & estimated Volume
	 ...
	 Vtrue = pow(2., NDIM-2.)*M_PI*integproduct; // true volume from analytic formula
      ...
      outfile << setprecision(4) <<"$"<< NDIM <<"$ & $"<< Vtot <<"$ & $"<< Vsphere <<"$ & $"<< Vtrue <<"$ & $"<< fabs(Vtrue-Vsphere)/Vtrue <<"$"<< endl;

//	 outfile << setprecision(9) << NDIM <<"\t"<< Ntrials <<"\t"<< Vsphere <<"\t"<< fabs(Vtrue-Vsphere)/Vtrue << endl;

//} // End of for loop for Ntrials

//	 outfile << "\n\n";
	 
} // End of for loop for dimension
	 
	 cerr << "data stored in hypersphere_table_latex.dat" << endl;
      outfile.close();
