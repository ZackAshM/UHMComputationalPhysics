...
float tbounce(float);	// half-time of a bounce cycle
float ybounce(float);	// height of a bounce

// start of main program -------------------
main()
{
    int k=0,n=0,Nmax;	// n = number of half bounces
    float C, yterm=0,ysum=0,tterm=0,tsum=0;	// C = restoration coeffcient factor; y/t term = dist / time on specific bounce, y/t sum = total dist / time 
...
{
    float t =  pow(2*Eb/m, 0.5)/g; // calculate the time for half a round trip
    return(t);
}
...
{
    float y = Eb/(m*g);  // calculate the distance for half a round trip
    return (y);
}
